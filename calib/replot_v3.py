...
def best_theta_at_time(t):
    ...
    return thetas[np.argmin(errs)]

theta_oracle = np.array([best_theta_at_time(t) for t in times_std])
...
plt.ylabel(r"$\theta$")
plt.title("Online θ Tracking under Drift + Abrupt Change")
plt.legend()
plt.grid(alpha=0.3)
plt.tight_layout()
plt.savefig(f"{prefix}_theta_history_with_oracle.png", dpi=300)
plt.show()
