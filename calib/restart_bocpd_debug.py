...
class BOCPD:
    ...
    def update(
        ...
        # 3) advance run_length + new r=0 expert
        for i, e in enumerate(self.experts):
        ...
        # 5) restart decision
        t_now = self.t + 1   # 新的时间步 index（1-based）
        r_old = self.restart_start_time
        anchor_rl = max(t_now - r_old, 0)

        # anchor expert: run_length == anchor_rl
        ...
        # run_length = 0 expert 的质量（当前时刻发生change的概率）
        ...
        if (
            ...
            if getattr(self.config, "use_backdated_restart", False):
                # Backdated: r <- s*, keep that expert
