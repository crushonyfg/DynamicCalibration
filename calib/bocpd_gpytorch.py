...
class BOCPD:
    def __init__(
        ...
        self.prev_dominant_rl: Optional[int] = None
        # self.prev_t: int = 0
    ...
    def _spawn_new_expert(self, pf_cfg: PFConfig, model_cfg: ModelConfig, dx: int,
        ...
        pf_new = ParticleFilter.from_prior(prior_sampler, pf_cfg, model_cfg.device, model_cfg.dtype, theta_anchor=self.theta_anchor)
    ...
    def _closest_expert_by_run_length(self, target_rl: int) -> Optional[Expert]:
    ...
    def update_batch(
        ...
        for e in self.experts:
            ...
            kernel = gpytorch.kernels.ScaleKernel(gpytorch.kernels.RBFKernel()).to(self.device, self.dtype)
        ...
        dominant_expert = self.experts[0]
        dominant_rl = dominant_expert.run_length
        
        if self.prev_dominant_rl is None:
            did_restart = False
        else:
            expected_rl = self.prev_dominant_rl + batch_size
            tol = int(getattr(self.config, "implicit_rl_tol", 1))
            did_restart = not (expected_rl - tol*batch_size <= dominant_rl <= expected_rl + tol*batch_size)
