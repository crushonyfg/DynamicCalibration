...
# def analyze_sudden_uni(seg_len_L, delta_mag, batch_size, seed, prefix):
#     store_dir = f"figs/sudden_{prefix}/{seg_len_L}_delta{delta_mag}_batch{batch_size}_seed{seed}"
#     os.makedirs(store_dir, exist_ok=True)
#     tag = f"L{seg_len_L}_delta{delta_mag:g}_bs{batch_size}_seed{seed}"
#     data = torch.load(f"C:/Users/yxu59/files/autumn2025/park/DynamicCalibration/sudden_grid_outputs/{prefix}/sudden_{tag}_results.pt", weights_only=False)
#     theta_stars = torch.load(f"C:/Users/yxu59/files/autumn2025/park/DynamicCalibration/sudden_grid_outputs/{prefix}/sudden_{tag}_phi_oracle.pt", weights_only=False)

#     # print("theta_stars:", type(theta_stars), theta_stars)
#     phi_hist, oracle_hist = theta_stars["phi_hist"], theta_stars["oracle_hist"]
#     # print("phi_hist, oracle_hist:", type(phi_hist), type(oracle_hist))
...
#     plt.grid(alpha=0.3)
#     plt.tight_layout()
#     plt.savefig(f"{store_dir}/sudden_{tag}_rmse.png")
...
#         others = data["R-BOCPD-PF-nodiscrepancy"]["others"]
#     else:
...
#     plt.grid(alpha=0.3)
#     plt.tight_layout()
#     plt.savefig(f"{store_dir}/sudden_{tag}_theta.png")
...
#     prefix = "v5_nodiscrepancy_usediscrepancy"
#     results = {}
...
#         results[(seg_len_L, delta_mag, batch_size, seed)] = analyze_sudden_uni(seg_len_L, delta_mag, batch_size, seed, prefix)
#     torch.save(results, f"figs/sudden_{prefix}/sudden_{prefix}_results.pt")
#     return results
...
def analyze_sudden_uni(
    seg_len_L, delta_mag, batch_size, seed,
    prefix1, prefix2=None, prefix3=None
):
    ...
    store_dir = f"figs/sudden_{prefix3}/{seg_len_L}_delta{delta_mag}_batch{batch_size}_seed{seed}"
    os.makedirs(store_dir, exist_ok=True)
    ...
    phi_hist = theta_stars["phi_hist"]
    oracle_hist = np.asarray(theta_stars["oracle_hist"], dtype=float)

    # -------- optional merge from prefix2 --------
    if prefix2 is not None:
        data2 = torch.load(
            f"sudden_grid_outputs/{prefix2}/sudden_{tag}_results.pt",
            weights_only=False
        )
        ...
        # replace PF with usediscrepancy
        if "R-BOCPD-PF-usediscrepancy" in data2:
            if "R-BOCPD-PF" in data:
                data.pop("R-BOCPD-PF")
            # if "R-BOCPD-PF-nodiscrepancy" in data:
            #     data.pop("R-BOCPD-PF-nodiscrepancy")

            data["R-BOCPD-PF-usediscrepancy"] = data2["R-BOCPD-PF-usediscrepancy"]

    # -------- RMSE trajectories --------
    names = list(data.keys())
    rmse_list = [data[n]["rmse"] for n in names]

    plt.figure(figsize=(8, 5))
    for n, rmse in zip(names, rmse_list):
        plt.plot(rmse, alpha=0.7, label=n)
    ...
    if "R-BOCPD-PF-nodiscrepancy" in data:
        others = data["R-BOCPD-PF-nodiscrepancy"]["others"]
    elif "R-BOCPD-PF-usediscrepancy" in data:
    ...
    plt.fill_between(t, lo, hi, color="red", alpha=0.2, label="R-BOCPD-PF 90% CI")
...
def main():
    ...
    results = {}
    for seg_len_L, delta_mag, batch_size, seed in itertools.product(
        seg_lens, magnitudes, batch_sizes, seeds
    ):
        results[(seg_len_L, delta_mag, batch_size, seed)] = analyze_sudden_uni(
            seg_len_L, delta_mag, batch_size, seed,
