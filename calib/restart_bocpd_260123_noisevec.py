...
class Expert:
    run_length: int
    pf: ParticleFilter
    delta_state: Optional[GPyTorchDeltaState]
...
class BOCPD:
    ...
    def _init_empty_delta_state(self, dx: int, model_cfg: ModelConfig):
    ...
    def update_batch(
        ...
        for e in self.experts:
            ...
            # var_particles = (weights[None,:] * diff.pow(2)).sum(dim=1)
            # rho = model_cfg.rho
            # var_theta = rho**2 * var_particles
            # noise_vec = model_cfg.delta_kernel.noise + var_theta
            ...
            # # mu_eta_all: [M, N]
            # # weights: [1, N]
            # eta_mix_all = (weights * mu_eta_all).sum(dim=1, keepdim=True)  # [M, 1]
            # # emulator mixture variance
            w = weights            # [1, M]
            eta_mix = (mu_eta_all * w).sum(dim=1)          # [N]
            eta_var_mix = ((mu_eta_all - eta_mix[:, None])**2 * w).sum(dim=1)  # [N]


            # eta_var_mix = (weights * (mu_eta_all - eta_mix_all) ** 2).sum(dim=1)  # [M]
            # physical observation noise
            # sigma_eps2 = 1e-4
            # total heteroscedastic noise for delta GP
            # noise_vec = (model_cfg.rho ** 2) * eta_var_mix  # [M]
            ...
            if use_noise_vec:
                ...
                delta_state.fit(max_iter=150, lr=0.05)
            else:
                delta_state = GPyTorchDeltaState(
                    X=X_hist,
                    y=resid_all,
